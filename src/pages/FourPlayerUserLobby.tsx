...
const FourPlayerUserLobby = () => {
  ...
  const [turn, setTurn] = useState<'player1' | 'player2' | 'player3' | 'player4' | null>(null);
  const [selectedStat, setSelectedStat] = useState<string | null>(null);
  const [roundWins, setRoundWins] = useState({ player1: 0, player2: 0, player3: 0, player4: 0 });
  ...
  const startBattle = () => {
    ...
    const players: ('player1' | 'player2' | 'player3' | 'player4')[] = ['player1', 'player2', 'player3', 'player4'];
  ...
  const handleStatSelection = (stat: string) => {
    ...
    const values = {
      ...
      player4: cardP4[stat as keyof typeof cardP4] as number
    };
    ...
    // Single winner case
    if (winners.length === 1) {
      const winner = winners[0] as keyof typeof decks;
      setRoundWins(prev => ({ ...prev, [winner]: prev[winner as keyof typeof prev] + 1 }));
      ...
      setTimeout(() => {
        ...
        const topCards = {
          ...
          player4: newDecks.player4.shift()!
        };
        
        // Add all cards to winner's deck
        newDecks[winner] = [...newDecks[winner], topCards.player1, topCards.player2, topCards.player3, topCards.player4];
  ...
  const moveToNextTurn = () => {
    ...
    else if (turn === 'player3') setTurn('player4');
            ...
            <div className="space-y-8">
              <div className="flex justify-between items-center">
                ...
                <div className="bg-fantasy-accent/20 p-3 rounded">
                  <p className="text-lg">
                    Turn: {turn === 'player1' ? 'Player 1' : turn === 'player2' ? 'Player 2' : 
                           turn === 'player3' ? 'Player 3' : 'Player 4'}
                  </p>
                </div>
              </div>
              ...
              <div className="bg-gray-800/50 p-4 rounded">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                  ...
                  <p>Player 4: {roundWins.player4}</p>
                </div>
              </div>
              
              {!gameOver && !selectedStat && (
                <div className="space-y-3">
                  <p className="text-center">
                    {turn?.replace('player', 'Player ')} select a stat to compare:
                  </p>
...
export default FourPlayerUserLobby;
