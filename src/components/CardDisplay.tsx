...
import { getElementColor, getCardImageUrl, getCardImageFallback } from "@/lib/api";
...
export const CardDisplay = ({ card, isSelected, onClick, showQuantity }: CardDisplayProps) => {
  const [imageError, setImageError] = useState(false);
  const elementColorClass = getElementColor(card.element);
    ...
    >
      ...
      <CardContent className="p-3">
        ...
        <div className="aspect-[3/4] mb-3 rounded-lg overflow-hidden bg-gray-700 flex items-center justify-center">
          {!imageError ? (
            <img
              src={getCardImageUrl(card.token_id)}
              alt={card.name}
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 gap-1 text-xs">
            <div className="flex justify-between text-gray-300">
              <span>Power:</span>
              <span className="text-red-400 font-semibold">{card.power}</span>
            </div>
            <div className="flex justify-between text-gray-300">
              <span>Health:</span>
