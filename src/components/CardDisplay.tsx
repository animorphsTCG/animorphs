...
const CardDisplay = ({
  ...
  return <div className="fantasy-card cursor-pointer" onClick={onClick}>
      <div className="fantasy-card-inner h-96 w-72">
        <div className="h-full w-full">
          <img src={imageUrl} alt={card.nft_name || `Card #${card.card_number}`} className="w-full h-full rounded-lg object-fill" />
        </div>
      </div>
    </div>;
};
