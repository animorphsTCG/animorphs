...
try {
    ...
    // Validate the file path exists in the database (try matching variations)
    ...
    if (!$row) {
        // Try removing "tcg.frontend/" prefix if present
    ...
    if (!file_exists($filePath)) {
        if ($filePath[0] === '/') {
            // If path is root-based and includes tcg.frontend, adjust to actual directory
            ...
            // Relative path: try relative to this directory
            $tryPath = __DIR__ . '/' . $filePath;
            if (file_exists($tryPath)) {
                $filePath = $tryPath;
            } else if (str_starts_with($filePath, 'tcg.frontend/')) {
                // Remove redundant directory prefix and try again
