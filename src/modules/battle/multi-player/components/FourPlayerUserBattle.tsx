...
export const FourPlayerUserBattle = () => {
    ...
    <div className="container mx-auto py-8 px-4">
      <Card className="border-2 border-fantasy-primary bg-black/70">
        <CardHeader>
          ...
          <CardDescription>User-created lobby with 4 human players</CardDescription>
        </CardHeader>
          ...
          {gameOver ? (
            ...
            <>
              ...
              {!isUserTurn && !selectedStat && (
                <div className="text-center bg-fantasy-primary/20 p-4 rounded">
                  <p className="text-lg">
                    Waiting for {participants.find(p => p.user_id !== user?.id && 
                      participants.find(p => p.user_id === winner)?.user_id === p.user_id)?.username || 'other player'} to select a stat...
                  </p>
                </div>
              )}
            </>
...
export default FourPlayerUserBattle;
