...
if($action==='choose_stat'){
  ...
  if($st['round']>10){
      ...
      try{
        ...
        up_stats($pdo,(int)$b['p1_user_id'],'1v1_random',$<?php
// 1v1 Random — full production version (rewards + end-of-match popup + lobby_status + MATCH_LAUNCH)
...
/* ---------- Helpers ---------- */
function is_participant(PDO $p,$l,$u){$s=$p->prepare("SELECT 1 FROM lobby_participants WHERE lobby_id=:l AND user_id=:u");$s->execute([':l'=>$l,':u'=>$u]);return(bool)$s->fetchColumn();}
function lobby_info(PDO $p,$l){$s=$p->prepare("SELECT id,owner_id,mode,status FROM lobbies WHERE id=:l");$s->execute([':l'=>$l]);return$s->fetch(PDO::FETCH_ASSOC);}
function random_deck(PDO $p){$r=$p->query("SELECT token_id FROM animorph_cards ORDER BY random() LIMIT 10");return array_column($r->fetchAll(PDO::FETCH_ASSOC),'token_id');}
function get_card(PDO $p,$id){$s=$p->prepare("SELECT token_id,display_name,card_image,power_rating,health,attack,sats,size,animorph_type FROM animorph_cards WHERE token_id=:t");$s->execute([':t'=>$id]);return$s->fetch(PDO::FETCH_ASSOC);}
function statcol($l){$m=['power'=>'power_rating','health'=>'health','attack'=>'attack','sats'=>'sats','size'=>'size'];return$m[strtolower($l)]??null;}
function load_battle(PDO $p,$l){$s=$p->prepare("SELECT * FROM battles WHERE lobby_id=:l ORDER BY id DESC LIMIT 1");$s->execute([':l'=>$l]);return$s->fetch(PDO::FETCH_ASSOC)?:null;}
...
// LOBBY STATUS (used by my_lobby.php to auto-redirect both players)
if ($action === 'lobby_status') {
    if (!$lobbyId || !is_participant($pdo, $lobbyId, $userId)) {
        echo json_encode(['success'=>false,'error'=>'bad_lobby']); exit;
    }
    $l = lobby_info($pdo, $lobbyId);
    if (!$l) { echo json_encode(['success'=>false,'error'=>'lobby_not_found']); exit; }

    $parts = $pdo->prepare("SELECT p.user_id,u.username,p.is_ready FROM lobby_participants p JOIN users u ON u.id=p.user_id WHERE p.lobby_id=:l ORDER BY p.joined_at ASC");
    $parts->execute([':l'=>$lobbyId]);
    $plist = array_map(function($r){
        return ['username'=>$r['username'], 'is_ready'=>($r['is_ready']==='t'||$r['is_ready']===true||(int)$r['is_ready']===1)];
...
if($action==='owner_start_match'){
  $l=lobby_info($pdo,$lobbyId); if(!$l||$l['owner_id']!=$userId){echo json_encode(['success'=>false,'error'=>'not_owner']);exit;}
  $p=$pdo->prepare("SELECT user_id,is_ready FROM lobby_participants WHERE lobby_id=:l ORDER BY joined_at ASC");
  $p->execute([':l'=>$lobbyId]);$ps=$p->fetchAll(PDO::FETCH_ASSOC);
  if(count($ps)<2){echo json_encode(['success'=>false,'error'=>'need_two']);exit;}
  // verify both ready (any truthy/t)
  $r0 = ($ps[0]['is_ready']==='t'||$ps[0]['is_ready']===true||(int)$ps[0]['is_ready']===1);
  $r1 = ($ps[1]['is_ready']==='t'||$ps[1]['is_ready']===true||(int)$ps[1]['is_ready']===1);
  if(!$r0 || !$r1){echo json_encode(['success'=>false,'error'=>'not_ready']);exit;}

  $p1=(int)$l['owner_id'];
  $p2=(int)($ps[0]['user_id']==$p1?$ps[1]['user_id']:$ps[0]['user_id']);

  $state=['p1'=>['user_id'=>$p1,'deck'=>random_deck($pdo)],
  ...
  $pdo->prepare("INSERT INTO battles(lobby_id,mode,p1_user_id,p2_user_id,status,state_json)
      ...
      ->execute([':l'=>$lobbyId,':p1'=>$p1,':p2'=>$p2,':s'=>json_encode($state)]);
  $pdo->prepare("UPDATE lobbies SET status='in_match',updated_at=now() WHERE id=:l")
      ->execute([':l'=>$lobbyId]);

  // ✅ broadcast launch signal so both clients redirect into the match immediately
  signal($pdo,$lobbyId,$userId,'MATCH_LAUNCH');
