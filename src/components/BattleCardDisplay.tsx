...
const BattleCardDisplay: React.FC<BattleCardDisplayProps> = ({
  ...
  return (
    <div className={`relative flex flex-col items-center ${isActive ? 'scale-105 z-10' : ''}`}>
      <div className="text-center mb-1 font-fantasy">
        ...
        <div className="bg-fantasy-accent/20 px-3 py-1 rounded-full text-sm font-bold">
          Wins: {roundWins}
          {cardCount !== undefined && (
            <span className="ml-2">Cards: {cardCount}</span>
          )}
        </div>
      </div>
      ...
      {isFlipped ? (
        card ? (
              ...
              <div className="absolute -bottom-12 left-0 right-0 flex justify-center gap-2 flex-wrap">
                {stats.map((stat) => (
                  <button
                    ...
                    className={`px-3 py-1 capitalize text-sm rounded-full ${
                      selectedStat === stat
                        ? "bg-fantasy-accent text-white"
                        : "bg-fantasy-primary/30 hover:bg-fantasy-primary/50"
                    }`}
                  >
                    {stat}
                  </button>
          ...
          <div className="h-96 w-72 flex items-center justify-center bg-gray-800/50 rounded">
            <p>No cards left</p>
          </div>
        )
      ) : card ? (
        <div className="h-96 w-72 bg-fantasy-primary/20 rounded shadow-md flex items-center justify-center">
